...
#if defined(BSD)        /* FreeBSD를 위한 회계 레코드 구조체 및 서식 */
#define acct acctv2
...
#endif
#if defined(LINUX)
#define acct acct_v3    /* Linux를 위한 회계 레코드 구조체 */
#endif
...
compt2ulong(comp_t comptime)    /* comt_t를 unsigned long으로 변환 */
{
    unsigned long   val;
    int             exp;

    val = comptime & 0x1fff;    /* 13비트 소수부 */
    exp = (comptime >> 13) & 7; /* 3비트 지수(0-7) */
    while (exp-- > 0)
        val *= 8;
    return(val);
}
